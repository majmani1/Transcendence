...
os.environ.setdefault('DJANGO_SETTINGS_MODULE', 'back_end_second_game.settings')

application = ProtocolTypeRouter({
    "http": get_asgi_application(),
    # "websocket": AuthMiddlewareStack(
    #     URLRouter(routing.websocket_url_p)
    # )
    'websocket': Auth_Middleware(AllowedHostsOriginValidator(
      AuthMiddlewareStack(URLRouter(routing.websocket_url_p))),
  )
